...
> **状态 / Status:** Sprint 2 (编辑器商业化) 已完成，准备进入 Sprint 3 (高级交互)
...
#### Step 1: 修复遗留问题 (1 天)
- [x] 修复 `koi_printer` 中的 flaky 测试
- [x] 清理 `docs/` 过期文档，合并为单一 `ROADMAP.md`
- [x] 在 `README.md` 中添加 pre-commit hook 自动化配置脚本
...
- [x] 封装 `VariableSelectDropdown` 组件，读取 `EditorState.schema`
- [x] 在 `TextElementInspector` 中增加"绑定变量"模式
- [x] 选中变量后，底层自动设置 `{{fieldKey}}`，UI 显示绿色胶囊标签
- [x] 扩展到 Barcode / QrCode Inspector

#### Step 3: 双模式预览 (2 天)
- [x] 在 `TopToolbar` 增加 编辑/预览 Toggle 开关
- [x] 编辑模式：`{{xxx}}` → 高亮中文别名标签 `<运单号>`
- [x] 预览模式：注入 `mockData` 渲染真实效果
- [x] `EditorState.isPreviewMode` 已存在，只需对接画布渲染

#### Step 4: ForEach 循环容器可视化 (3 天)
- [x] 左侧面板增加"循环容器"组件
- [x] `CenterCanvas` 中渲染为虚线边框容器
- [x] 支持将子元素拖入循环容器内部
- [x] 升级 `EditorState` 支持嵌套级联的拖拽排序
