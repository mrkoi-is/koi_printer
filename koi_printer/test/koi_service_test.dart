...
void main() {
  ...
  group('KoiPrintJobQueue', () {
    test('KoiPrintJobQueue uses delay profiles correctly', () async {
      ...
      const config2018 = KoiPrintConfig(
      ...
      final p1 = queue.enqueue(
        KoiPrintJob(documents: [doc], config: config2021),
      );
      final p2 = queue.enqueue(
        KoiPrintJob(documents: [doc], config: config2021),
      );
      await Future.wait([p1, p2]);

      // Queue 2 jobs with 2018 profile (500ms delay between them)
      final p3 = queue.enqueue(
        KoiPrintJob(documents: [doc], config: config2018),
      );
      final p4 = queue.enqueue(
        KoiPrintJob(documents: [doc], config: config2018),
      );
