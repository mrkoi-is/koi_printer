...
## CHECKPOINT 1: 面单模式绝对坐标拖拽 (Absolute Positioning Editor) [DONE]
...
**验收:**
- [x] 面单模式下的组件能在画布中自由拖拽移动。
- [x] 拖拽结束后，右侧属性检查器中的 x, y 坐标会随之更新。
- [x] 撤销/重做 (Undo/Redo) 支持拖拽坐标的恢复。

---

## CHECKPOINT 2: 云端保存与分发 API (Cloud Sync API) [DONE]
...
**验收:**
- [x] 能够通过网络请求获取远端模板列表。
- [x] 当前画布能序列化为 JSON 并通过 API 上传成功。
- [x] 编辑器具备“云端”和“本地”模板切换能力。

---

## CHECKPOINT 3: 真机远程打样 (Remote Proofing) [DONE]
...
**验收:**
- [x] 提供直观的远程连接方式（局域网 IP 直连或扫码连网）。
- [x] 一键触发真机打样，设备端能顺利接收 JSON 并解析打印。
- [x] 断线/发送失败时提供清晰的错误提示。
