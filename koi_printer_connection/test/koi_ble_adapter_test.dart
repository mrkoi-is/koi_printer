...
void main() {
  ...
  group('KoiBleAdapter', () {
    ...
    setUp(() {
      ...
      when(() => mockDevice.connectionState).thenAnswer((_) => Stream.value(BluetoothConnectionState.disconnected));
    ...
    test('connect matches specific service and characteristic UUIDs', () async {
      ...
      when(() => mockDevice.discoverServices()).thenAnswer((_) async => [mockService1, mockService2]);
    ...
    test('sendChunks writes to characteristic', () async {
      ...
      when(() => mockChar.write(any(), withoutResponse: any(named: 'withoutResponse')))
          .thenAnswer((_) async {});
      ...
      verify(() => mockChar.write([0x01, 0x02], withoutResponse: true)).called(1);
      verify(() => mockChar.write([0x03, 0x04], withoutResponse: true)).called(1);
    ...
    test('sendChunks throws error and catches it', () async {
      ...
      await expectLater(adapter.sendChunks([[0x01, 0x02]]), throwsException);
      await connectionStateController.close();
    });
    ...
    test('ignores system UUIDs when searching for characteristics', () async {
      ...
      when(() => sysService.uuid).thenReturn(Guid('00001800-0000-1000-8000-00805f9b34fb'));
      when(() => sysChar.uuid).thenReturn(Guid('00002a00-0000-1000-8000-00805f9b34fb'));
      when(() => sysChar2.uuid).thenReturn(Guid('00002a01-0000-1000-8000-00805f9b34fb'));
      ...
      when(() => normalService.characteristics).thenReturn([sysChar2, normalChar]);
      when(() => mockDevice.discoverServices()).thenAnswer((_) async => [sysService, normalService]);
      when(() => mockDevice.connect(timeout: any(named: 'timeout'), autoConnect: any(named: 'autoConnect'), mtu: any(named: 'mtu')))
          .thenAnswer((_) async {});
